        ...
        while len(moves_dict) > 1:
            for move_dict in moves_dict:
                ...
                for i in range(playouts):
                    move_dict['Victories'] += self.make_playout(new_context)
...
class ShotAgent(SequentialHalvingAgent):
    ...
    def shot(self, board, budget, budget_used=0, playouts=0, wins=0, at_root=False):
        legal_moves = self.game.moves(board, self.player)
        ...
        if budget == 1:
            updated_wins = wins + self.make_playout(board)
        ...
        if len(moves) == 1:
            ...
            move, new_budget_used, new_playouts, new_wins = self.shot(new_board, budget)
            return move, budget_used+new_budget_used, playouts+new_playouts, wins+new_wins

        if self.transposition_table.keys().isdisjoint([str(board)]):
            playouts_dict = {}
            victories_dict = {}
            for legal_move in legal_moves:
                playouts_dict[str(legal_move)] = 0
                victories_dict[str(legal_move)] = 0
            self.transposition_table[str(board)] = {'budget_node': 0, 'playouts': playouts_dict, 'victories': victories_dict}
        t = self.transposition_table[str(board)]

        if t['budget_node'] <= len(moves):
            for move in moves:
                if t['playouts'][str(move)] == 0:
                    new_board = self.game.apply(move, board)
                    win = self.make_playout(new_board)
                    playouts += 1
                    budget_used += 1
                    wins += win
        ...
        while len(moves) > 1:
            ...
            for m in range(len(moves)-1, -1, -1):
                if t['playouts'][str(moves[m])] < b:
                    ...
                    move, new_budget_used, new_playouts, new_wins = self.shot(new_board, b1)
    ...
    def select_action(self, game, context, max_seconds=0, max_iterations=0, max_depth=0):
        move, b_u, p, w = self.shot(context, 2048, at_root=True)
        return move